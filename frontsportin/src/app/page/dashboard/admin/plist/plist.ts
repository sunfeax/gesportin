...
@Component({
  selector: 'app-dashboard-admin-plist-page',
  standalone: true,
  imports: [CommonModule, FormsModule, KpiCardComponent, BaseChartDirective],
  templateUrl: './plist.html',
  styleUrl: './plist.css',
})
export class DashboardAdminPlistPage implements OnInit {
  ...
  private cargarClubes(): void {
    this.clubService.getPage(0, 100).subscribe({
      next: (page) => {
        this.clubes.set(page.content);
        if (page.content.length > 0) {
          this.clubSel.set(page.content[0].id);
          this.cargarTemporadas(page.content[0].id);
        } else {
          this.cargando.set(false);
          this.error.set('No hay clubes disponibles');
        }
      },
      error: () => {
        this.cargando.set(false);
        this.error.set('Error al cargar la lista de clubes');
      },
    });
  }

  private cargarTemporadas(idClub: number): void {
    this.temporadaService.getPage(0, 100, '', '', '', idClub).subscribe({
      next: (page) => {
        ...
        if (page.content.length > 0) {
          ...
          this.stats.set(null);
          this.temporadaSel.set(null);
          this.cargando.set(false);
          this.error.set('Este club no tiene temporadas registradas');
        }
      },
      error: () => {
        this.cargando.set(false);
  ...
  alCambiarClub(idClub: number): void {
    this.clubSel.set(Number(idClub));
    this.cargarTemporadas(Number(idClub));
  }
